    ...
    <section className={cn("rounded-2xl border border-white/10 bg-black/25 backdrop-blur p-6", className)}>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
        ...
        <ul className="space-y-2 text-sm text-gray-200/80">
          <li>
            <span className="font-semibold text-white">{aiLabel}</span> Gorilla Parlays (AI) <span className="text-gray-400">{aiPeriod}</span>
          </li>
          <li>
            <span className="font-semibold text-white">{customLabel}</span> Gorilla Parlay Builder builds {customPeriod && <span className="text-gray-400">{customPeriod}</span>}
