  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/30 backdrop-blur-sm p-4 mb-3">
      ...
      <ul className="text-xs text-white/70 space-y-2">
        {highest && (
          <li>
            <span className="text-white/50">Highest confidence: </span>
            <Link href={`/analysis/${highest.slug}`} className="text-emerald-400/90 hover:underline">
              {highest.matchup}
            </Link>
            <span className="text-white/60"> ({Math.round(highest.confidence)}%)</span>
          </li>
        )}
        {balanced && balanced !== highest && (
          <li>
            <span className="text-white/50">Most balanced: </span>
            <Link href={`/analysis/${balanced.slug}`} className="text-emerald-400/90 hover:underline">
              {balanced.matchup}
            </Link>
            <span className="text-white/60"> ({Math.round(balanced.confidence)}%)</span>
          </li>
        )}
        {edge && edge !== highest && (
          <li>
            <span className="text-white/50">Largest edge: </span>
            <Link href={`/analysis/${edge.slug}`} className="text-emerald-400/90 hover:underline">
              {edge.matchup}
            </Link>
            {edge.edgeLabel && (
              <span className="text-white/60"> — {edge.edgeLabel}</span>
            )}
          </li>
        )}
      </ul>
