...
/**
 * Tracks which verification records have already shown the “success” celebration,
 * so we don't spam users when polling refreshes or navigating between tabs.
 */
export class VerificationCelebrationRegistry {
  private readonly storageKey: string
  private readonly maxEntries: number
  private readonly seen = new Set<string>()

  constructor(private readonly storage: KeyValueStorage | null, options?: Omit<Options, "storage">) {
    this.storageKey = options?.storageKey || "pg_verification_record_celebrations_v1"
    this.maxEntries = Math.max(10, options?.maxEntries ?? 50)
    this.hydrate()
  }

  static fromSessionStorage(): VerificationCelebrationRegistry {
    const storage =
      typeof window !== "undefined" && window.sessionStorage
        ? (window.sessionStorage as unknown as KeyValueStorage)
        : null
    return new VerificationCelebrationRegistry(storage)
  }
