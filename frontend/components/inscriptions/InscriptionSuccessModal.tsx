...
export type InscriptionSuccessModalPayload = {
  savedParlayId: string
  parlayTitle?: string
  solscanUrl: string
  inscriptionTx?: string | null
}

type Props = {
  open: boolean
  payload: InscriptionSuccessModalPayload | null
  onClose: () => void
}
...
export function InscriptionSuccessModal({ open, payload, onClose }: Props) {
  useEffect(() => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Parlay inscribed"
        data-testid="inscription-success-modal"
      >
        <motion.div
        ...
        >
            ...
            <div className="mb-4 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-left">
              ...
              <div className="mt-1 flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await navigator.clipboard.writeText(payload.inscriptionTx || "")
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <Button asChild className="w-full bg-emerald-500 text-black hover:bg-emerald-400">
              <Link href={payload.solscanUrl} target="_blank" rel="noreferrer">
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  await navigator.clipboard.writeText(payload.solscanUrl)
                  toast.success("Solscan link copied")
                } catch {
                  toast.error("Failed to copy")
                }
              }}
