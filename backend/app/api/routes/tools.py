...
async def get_heatmap_probabilities(
    ...
    Always returns 200 with a list (possibly empty). Never 500s on empty or no-data.
    ...
    try:
        ...
        for game in games:
            try:
                ...
                logger.warning("heatmap_probabilities game_error game_id=%s error=%s", game.id, e)
                responses.append(HeatmapProbabilityResponse(
                    game_id=str(game.id),
                    home_win_prob=0.5,
                    away_win_prob=0.5,
        ...
        logger.exception("heatmap_probabilities unexpected_error sport=%s", sport)
        return []
