"""Soccer adapter: normalize availability, efficiency, matchup from matchup_data."""
...
class SoccerAdapter(BaseUgieAdapter):
    """Soccer: availability (injury/suspension), efficiency (goals/rating), matchup traits."""
    ...
    def normalize(self, matchup_data: Dict[str, Any], game: Any = None) -> NormalizedUgieInputs:
        ...
        if home_impact is not None and away_impact is not None:
            ...
            out.availability = {
                ...
                "why_summary": f"Injury/suspension impact home {h:.2f}, away {a:.2f}.",
            }
        else:
        ...
        home_rating = (home_stats.get("strength_ratings") or {}).get("overall_rating") if isinstance(home_stats.get("strength_ratings"), dict) else home_stats.get("overall_rating") or home_stats.get("goals_per_game")
        away_rating = (away_stats.get("strength_ratings") or {}).get("overall_rating") if isinstance(away_stats.get("strength_ratings"), dict) else away_stats.get("overall_rating") or away_stats.get("goals_per_game")
        if home_rating is not None and away_rating is not None:
            ...
            out.efficiency = {
                ...
                "away_rating": a,
                "signals": [
                    {"key": "home_rating", "value": h, "weight": 0.5, "direction": "home", "explain": "Goals/strength"},
                    {"key": "away_rating", "value": a, "weight": 0.5, "direction": "away", "explain": "Goals/strength"},
                ],
                "confidence": 0.85,
                "why_summary": f"Home {h:.1f}, away {a:.1f} (goals/rating).",
            }
        else:
