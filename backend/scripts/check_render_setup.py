...
def main():
    ...
    print("\n" + "="*60)
    print("  🚀 Render Deployment Setup Checker")
    print("="*60)
    ...
    required_vars = {
        "THE_ODDS_API_KEY": {
            ...
            "where": "https://the-odds-api.com - Dashboard → API Keys",
    ...
    for var_name, info in required_vars.items():
        ...
        if not has_value or "Placeholder" in status:
            print(f"      📍 Get it from: {info['where']}")
            if "notes" in info:
                print(f"      💡 {info['notes']}")
    
    # URLs (need to be set in Render)
    print_section("🌐 DOMAIN URLs (Set in Render Dashboard)")
    ...
    for var_name, info in url_vars.items():
        ...
        if "localhost" in current or "127.0.0.1" in current:
            print_item(var_name, "⚠️  Using localhost (update for Render)")
            print(f"      💡 Render default: {info['render_default']}")
            print(f"      📝 Set this in Render dashboard after deployment")
        else:
            print_item(var_name, f"✅ {current}")
    
    # Optional but recommended
    print_section("🟡 OPTIONAL (But Recommended)")
    ...
    # Inscriptions (Solana)
    print_section("⛓️  INSCRIPTIONS WORKER (Optional)")
    
    inscriptions_vars = {
        "SIGNER_PRIVATE_KEY": {
            "value": os.getenv("SIGNER_PRIVATE_KEY"),
            "where": "Generate Solana wallet or use existing",
            "notes": "⚠️  Keep secret! Never commit to git"
        },
        "RPC": {
            "value": os.getenv("RPC"),
    ...
    print_section("✅ AUTO-WIRED BY RENDER (No Action Needed)")
    ...
    print(f"  📈 You have {have_count}/{total_count} optional variables set")
    print(f"  ✅ All required variables are ready!")
    print(f"\n  🚀 Next steps:")
