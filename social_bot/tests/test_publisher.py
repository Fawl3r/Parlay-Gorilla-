...
def test_publisher_attaches_media_when_upload_succeeds(monkeypatch, tmp_path) -> None:
    image_path = tmp_path / "img.png"
    image_path.write_bytes(b"\x89PNG\r\n\x1a\n")  # minimal header; content is not parsed by uploader
    captured = {"tweet_payload": None}
    ...
    class FakeClient:
        ...
        def post(self, url, headers=None, json=None, files=None):
            req = httpx.Request("POST", url)
            if "media/upload" in str(url):
                return httpx.Response(200, json={"media_id_string": "999"}, request=req)
            captured["tweet_payload"] = json
            return httpx.Response(201, json={"data": {"id": "123"}}, request=req)
    ...
    res = pub.publish_single(text="hello", image_path="img.png")
    assert res.success is True
    assert captured["tweet_payload"]["media"]["media_ids"] == ["999"]


def test_publisher_posts_text_only_when_upload_fails(monkeypatch, tmp_path) -> None:
    image_path = tmp_path / "img.png"
    image_path.write_bytes(b"\x89PNG\r\n\x1a\n")
    captured = {"tweet_payload": None}
    ...
    class FakeClient:
        ...
        def post(self, url, headers=None, json=None, files=None):
            req = httpx.Request("POST", url)
            if "media/upload" in str(url):
                return httpx.Response(500, json={"error": "nope"}, request=req)
            captured["tweet_payload"] = json
            return httpx.Response(201, json={"data": {"id": "123"}}, request=req)
    ...
    res = pub.publish_single(text="hello", image_path="img.png")
    assert res.success is True
    assert "media" not in captured["tweet_payload"]
