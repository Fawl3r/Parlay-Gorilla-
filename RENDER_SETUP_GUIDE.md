...
- ✅ **Status**: Render will generate this automatically via `render.yaml`
...
5. **Set up Webhook**:
   ...
   - Copy webhook secret → `LEMONSQUEEZY_WEBHOOK_SECRET`
6. **Create Credit Pack Products**:
   - Create products for: 10 credits, 25 credits, 50 credits, 100 credits
   - Copy each Variant ID → `LEMONSQUEEZY_CREDITS_*_VARIANT_ID`
...
2. **Create API Key**:
   ...
   - Create new key
   - Copy → `COINBASE_COMMERCE_API_KEY`
3. **Set up Webhook**:
   - Go to Settings → Webhooks
   - Add webhook URL: `https://api.parlaygorilla.com/api/webhooks/coinbase`
   - Copy webhook secret → `COINBASE_COMMERCE_WEBHOOK_SECRET`
...
- ✅ `NEXT_PUBLIC_SITE_URL` = `https://www.parlaygorilla.com` (from render.yaml)
- ✅ `NEXT_PUBLIC_API_URL` = `https://api.parlaygorilla.com` (from render.yaml)
...
## 🔍 Quick Reference: Where to Find Your Keys
