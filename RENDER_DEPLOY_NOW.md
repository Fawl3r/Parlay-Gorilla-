...
4. **Render will detect `render.yaml`** and show you a preview of what will be created:
   - ✅ `parlay-gorilla-postgres` (PostgreSQL database)
   - ✅ `parlay-gorilla-redis` (Redis/Key Value store)
   - ✅ `parlay-gorilla-backend` (FastAPI backend)
   - ✅ `parlay-gorilla-frontend` (Next.js frontend)
   - ✅ `parlay-gorilla-inscriptions-worker` (optional worker)
5. **Click "Apply"** to create everything

**⏱️ This takes 5-10 minutes.** Render will:
- Create the database
- Create Redis
...
Go to `parlay-gorilla-frontend` service → **Environment** tab:
...
1. **Check logs**: Go to frontend service → **Logs** tab
2. **Common issues**:
   - Build timeout → Upgrade to `starter` plan (free plan has limits)
...
2. Add CNAME records:
   - `www` → `parlay-gorilla-frontend.onrender.com`
   - `api` → `parlay-gorilla-backend.onrender.com`
3. In Render, go to each service → **Settings** → **Custom Domains**
4. Add your domain
...
**Once all checked, you're ready to sign up for LemonSqueezy!** 🎉
